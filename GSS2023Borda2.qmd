...
summary <- filtered.dat %>% summarize.vote.ranks(BordaSum)
top.results <- summarize.top.n.ranks(summary, filtered.dat)
dot.plot(summary, BordaSum)
...
```{r}
joint.summary.Borda <- joint.summary %>%
  unite(col = "Issues", c(Issues, Category), sep = ":") %>%
  dplyr::select(Issues, SummaryRank, ExpectedBorda) %>%
  arrange(Issues)
```
...
```{r}
joint.summary %>%
  filter(ExpectedBorda >= min(top.joint$ExpectedBorda) & 
...
joint.summary %>%
  filter(ExpectedBorda >= min(top.joint$ExpectedBorda) & 
           !SummaryRank %in% c(1,2,3)) %>%
  unite(col = "Issues", c(Issues, Category), sep = ":") %>%
  arrange(desc(ExpectedBorda)) %>%
  dplyr::select(Issues, ExpectedBorda, everything())
```
...
c1 <- joint.summary %>%
  ...
  calculate.t.score(ExpectedBorda) %>%
  unite(col = "Issues", c(Issues, Category), sep = ":") 
...
```{r}
c3  %>%
  dplyr::select(Issues, ExpectedBorda, `T-Score`, everything()) %>%
  arrange(desc(ExpectedBorda)) %>%
  calculate.t.score(ExpectedBorda)

joint.summary %>%
  filter(Issues == "Mentoring/Advisor") %>%
  dplyr::select(Category, ExpectedBorda, `T-Score`, everything()) %>%
  arrange(desc(ExpectedBorda)) %>%
  calculate.t.score(ExpectedBorda) %>%
  slice(-1) %>%
  mutate(AvgT = mean(`T-Score`)) %>%
  dplyr::select(AvgT, everything())
```
